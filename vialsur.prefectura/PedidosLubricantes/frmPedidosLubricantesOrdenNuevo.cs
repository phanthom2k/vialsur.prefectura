...
{
    public partial class frmPedidosLubricantesOrdenNuevo : Form
    {
        public frmPedidosLubricantesOrdenNuevo()
        {
            InitializeComponent();
        ...
        private void frmPedidosLubricantesOrdenNuevo_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        entidades.vialsur.prefectura.detalle_pedidos_aceite detalle;
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("¿Desea guardar la información?", "Alerta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes && verificar())
                {
                    ...
                    if (EsNuevo)
                        ///llamo a insert
                        new logica.vialsur.prefectura.Catalogos.cls_logica_detalle_pedidos_aceite().InsertarDetallePedido(detalle);
                    else
                        ///llamo a update
                        new logica.vialsur.prefectura.Catalogos.cls_logica_detalle_pedidos_aceite().Actualizar(detalle);
