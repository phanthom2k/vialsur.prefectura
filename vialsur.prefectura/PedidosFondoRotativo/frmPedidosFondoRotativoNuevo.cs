...
{
    public partial class frmPedidosFondoRotativoNuevo : Form
    {
        public frmPedidosFondoRotativoNuevo()
        {
            InitializeComponent();
        ...
        entidades.vialsur.prefectura.fondo_detalle_pedido detalle;
        ...
        private void frmPedidosFondoRotativoNuevo_Load(object sender, EventArgs e)
        {
            try
            {
                if (EsNuevo)
                {
                    detalle = new entidades.vialsur.prefectura.fondo_detalle_pedido();
                    //detalle.pedidos_id = PedidoId;
                    detalle.fondo_pedido_id = PedidoId;
                }
                else
                {
                    //detalle = new logica.vialsur.prefectura.Catalogos.cls_logica_detalle_pedidos().ConsultarDetallePedidoById(DetallePedidoId);
                    detalle = new logica.vialsur.prefectura.Catalogos.cls_logica_fondo_detalle_pedido().ConsultarDetallePedidoById(DetallePedidoId);
            ...
            try
            {
                if (MessageBox.Show("¿Desea guardar la información?", "Alerta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes && verificar())
                {
                    ...
                    if (EsNuevo)
                        ...
                        new logica.vialsur.prefectura.Catalogos.cls_logica_fondo_detalle_pedido().InsertarDetallePedido(detalle);
                    else
                        ///llamo a update
                        //new logica.vialsur.prefectura.Catalogos.cls_logica_detalle_pedidos().Actualizar(detalle);
                        new logica.vialsur.prefectura.Catalogos.cls_logica_fondo_detalle_pedido().Actualizar(detalle);
        ...
        private void frmPedidosFondoRotativoNuevo_KeyDown(object sender, KeyEventArgs e)
        {
