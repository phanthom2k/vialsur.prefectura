...
namespace datos.vialsur.prefectura
{
    ...
    public class cls_data_pedidos_aceite
    {
     ...
     /////   private string sql_Consulta = "SELECT id, nombre, obserbacion FROM ve_vehiculo_marca where activo=1 order by nombre asc;";
        ...
        public void Insertar(entidades.vialsur.prefectura.pedidos_aceite pedido)
        {            
            try
            {
                ...
                string _sql_insert = "INSERT INTO[dbo].[pedidos_aceite]([id],[fecha],[cedula],[orden_id],[aprobada]) VALUES(dbo.FUN_PEDIDOS_ACEITE_SEC(),@fecha,@cedula,@orden_id,0); "; 
        ...
        {            
            try
            {
                ...
                string _sql_UPDATE = "UPDATE[dbo].[pedidos_aceite] SET [observaciones] = @observaciones  WHERE[orden_id] = @orden_id;";
            ...
            {
                throw new Exception("No se pudo registrar los datos del pedido de lubricantes", ex);
            }
        }
        /// <summary>
        ...
        public void ActualizarEstado(int pedido_id, bool estado, string cedula)
        {
            try
            {
                ...
                string _sql_UPDATE = "UPDATE[dbo].[pedidos_aceite] SET [aprobada] = @aprobada, [cedula_autoriza] =@cedula_autoriza   WHERE [id] = @pedido_id;";
        ...
        public entidades.vialsur.prefectura.pedidos_aceite Consultar_Pedido(string ByOrdenID )
        {
            try
            {
                string consulta = "SELECT [id],[fecha],[cedula],[observaciones],[orden_id],[aprobada] FROM [dbo].[pedidos_aceite] WHERE [orden_id] = @orden_id";

                SqlParameter parametro = new SqlParameter("@orden_id", SqlDbType.VarChar,10 );
                parametro.Value = ByOrdenID.Replace(" ", "");

                entidades.vialsur.prefectura.pedidos_aceite obj_pedido = new entidades.vialsur.prefectura.pedidos_aceite();
        ...
        public entidades.vialsur.prefectura.pedidos_aceite Consultar_PedidoById(int ID)
        {
            try
            {
                string consulta = "SELECT [id],[fecha],[cedula],[observaciones],[orden_id],[aprobada] FROM [dbo].[pedidos_aceite] WHERE [id] = @id";
        ...
        public DataTable ObtenerOrdenesByEstado_UI(entidades.vialsur.prefectura.Orden_TipoEstadoPedido estado, string cedula="")
        {
            string consulta_sql =
                                ...
                                "SELECT pedidos_aceite.id, pedidos_aceite.fecha, pedidos_aceite.cedula, pedidos_aceite.observaciones, pedidos_aceite.orden_id, pedidos_aceite.aprobada, CONCAT(per_persona.apellidos,', ',per_persona.nombres) solicitante, " +
                                "vehiculo.codigo, vehiculo.placa,  marca.nombre, modelo.modelo, '-----' autoriza " +
                                "FROM   pedidos_aceite, per_persona, orden, ve_vehiculo_responsable, ve_vehiculo vehiculo, ve_vehiculo_modelo modelo,ve_vehiculo_marca marca " +
                                "WHERE pedidos_aceite.cedula = per_persona.cedula AND pedidos_aceite.orden_id = orden.id AND orden.ve_vehiculo_responsable_id = ve_vehiculo_responsable.id " +
                                "AND ve_vehiculo_responsable.ve_vehiculo_id = vehiculo.id AND  " +
                                "vehiculo.ve_vehiculo_modelo_id = modelo.id AND modelo.ve_vehiculo_marca_id = marca.id AND pedidos_aceite.aprobada=@aprobada "

                                :

                                "SELECT pedidos_aceite.id, pedidos_aceite.fecha, pedidos_aceite.cedula, pedidos_aceite.observaciones, pedidos_aceite.orden_id, pedidos_aceite.aprobada, CONCAT(per_persona.apellidos,', ',per_persona.nombres) solicitante, " +
                                "vehiculo.codigo, vehiculo.placa,  marca.nombre, modelo.modelo, CONCAT(p2.apellidos,', ', p2.nombres) autoriza " +
                                "FROM   pedidos_aceite, per_persona, orden, ve_vehiculo_responsable, ve_vehiculo vehiculo, ve_vehiculo_modelo modelo,ve_vehiculo_marca marca, per_persona p2 " +
                                "WHERE pedidos_aceite.cedula = per_persona.cedula AND pedidos_aceite.orden_id = orden.id AND orden.ve_vehiculo_responsable_id = ve_vehiculo_responsable.id " +
                                "AND ve_vehiculo_responsable.ve_vehiculo_id = vehiculo.id AND p2.cedula=pedidos_aceite.cedula_autoriza AND " +
                                "vehiculo.ve_vehiculo_modelo_id = modelo.id AND modelo.ve_vehiculo_marca_id = marca.id AND pedidos_aceite.aprobada=@aprobada ";
            ...
            if (cedula != "")
            {
                consulta_sql += " AND pedidos_aceite.cedula = @cedula";
            ...
            {
                throw new Exception("Error al consultar los datos de las ordenes de pedidos de lubricantes: " + ex.Message);
            }
        }
